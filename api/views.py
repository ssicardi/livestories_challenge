...
def countEvent(request, event, count=1):
	t = timezone.now()
	if count == 1:
		event_instance = Events.objects.create(event_type=event, date=t)
	else:
...
def getEventsFromDates(request):
	try:
		...
		jsonDict = {"result": [{"date": str(d["date__date"]), "event":d["event_type"], "count": d["count"]} for d in queryResult]}
